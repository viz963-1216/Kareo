...
## Blocked: `active` not set (5)

| ProviderService ID | Reason |
|---|---|
| PSV-NTPC-HC-001-HOME_CARE | SRC-002 is registered only as `https://www.health.ntpc.gov.tw/` (homepage), and no raw file is in the repo. Current service status cannot be confirmed. |
| PSV-NTPC-HC-002-HOME_CARE | Same as above. |
| PSV-NTPC-HC-003-HOME_CARE | Same as above. |
| PSV-NTPC-HC-004-HOME_CARE | Same as above. |
| PSV-NTPC-HC-005-HOME_CARE | Same as above. |

Next action: record the specific SRC-002 list URL or attachment (新北市居家長照機構名冊) in
`sources/source-registry.md`, confirm each of the 5 Providers against it, and then set `active`.
