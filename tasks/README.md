...
| B-008 Knowledge Foundation | MERGED（#26） | 未確認：尚無 PUBLISHED 版本；D-03／D-10 延伸使用無核准紀錄 | — | Jerry 補核准（集中清單 #2） |
| **B-011a Session／歸屬保護** | READY；未見提交 | — | B-004 ✅、B-008 ✅ | ▶ **下一個 Active Task** |
| B-010 規則引擎＋正式知識接線（含補助說明、location 驗證） | QUEUED；未見提交 | — | B-011a | 規則表 D-01a r2 確認後才算正式驗收 |
...
| **C-005 補助說明顯示＋位置三情境＋Lead＋UX** | READY；未見提交 | — | C-004 ✅ | ▶ 可立即以 Mock 開工 |
...
| J-002 規格／知識／隱私／Lead／位置 | r1–r3 DOC-MERGED（#19、#28）；**r4 送審中**；D-02 知識 0 筆核准 | 見「待 Jerry 決定」 |
...
|---|---|---|---|
| 1 | 同意 → **真實初評**與可能適用的制度／補助說明 | Jerry：D-02 審核、D-04；J-003：首次知識發布；B：B-011a → B-010 | C：C-005（Mock）；A：A-003-r2、A-005 |
| 2 | **行政區／縣市推薦** → Provider 詳情 → Google Maps | B-005；J-003：Provider 正式匯入、推薦路由 | Jerry：D-13、D-14 決定 |
| 3 | **我要媒合**與內部接件 | B-006；C-005 合併；Jerry：D-06 接件人 | J-002：地方補助知識 KP-*-002 |
...
A-004 ✅ ─┬─→ A-003-r2（已驗證座標）──────────────┐（真實 DISTANCE 案例）
          └─→ A-005（QA 案例）──────────────────┐ │
B-004 ✅ ─┐                                      │ │
B-008 ✅ ─┼─→ B-011a ─┬─→ B-010（規則引擎＋知識）│ │
          │           └─→ B-005 ─→ B-006         │ │
          └─→ B-009（每日 00:10）                │ │
B-005＋B-006＋B-010 ─→ B-011b                    │ │
C-004 ✅ ─→ C-005（Mock）                        │ │
J-002：D-02 內容核准 ─→ J-003 首次知識發布 ─→ B-010 真實 smoke
全部必要模組＋A-005＋A-003-r2 ─→ J-003 完整 E2E（release）─→ J-004 release
```

關卡：

- 首次知識發布只依賴 D-02 核准＋B-008（已合併），不等待最終 E2E，避免與 B-010 循環。
- B-006 依賴 B-005（Lead 需驗證 recommendationId），B-005 不依賴 B-006。
- 規格 PROPOSED 項目（D-01a、D-04、D-13、D-14）只允許可逆實作；**模組可以合併，但正式驗收需核准**。
...
完整清單（每項附建議與未決定的影響）集中在 `docs/MVP_DECISIONS.md`「待 Jerry 決定（集中清單）」。最影響真實流程的前三項：

1. **D-02 首批 9 筆知識逐筆審核**：沒有 PUBLISHED 版本，真實評估與補助說明都無法驗收。
2. **D-03 格式與 D-10 延伸使用補核准**：B-008 已合併但無核准紀錄。
3. **D-04 Session 安全 contract**：B-011a 已可開工，核准後才算正式驗收。
...
1. 下一個 Active Task：B-011a（TASK-B-011）。`generateId()` 的 `Math.random()` 不得用於 token。
...
| 日期 | 目標 |
|---|---|
| 9/26 | D-02 知識審核、D-03／D-04 決定；首次知識發布（整合環境可用時） |
| 9/30 | B-011a 合併；D-13／D-14 決定；地方補助知識 KP-*-002 送審 |
