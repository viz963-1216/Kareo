...
Owner: Jerry
Status: PROPOSED（D-06）；主要接件人：蘇子傑，週一至週五 09:00–21:00；**不設備援接件人**（Jerry 2026-09-24 決定）
...
| 角色 | 人選 | 責任 | 權限 |
|---|---|---|---|
| 主要接件人（Lead Operator） | **蘇子傑**（Jerry 本人，2026-09-24 指定；週一至週五 09:00–21:00） | 依 §5 時程聯繫、更新狀態、記錄結果 | `lead:list`、`lead:show`、`lead:reveal-contact`（被指派案件）、`lead:update` |
| 備援接件人 | **不設**（Jerry 2026-09-24 決定）。風險：主要接件人無法處理時沒有人接手，改以「暫停對外媒合入口」處理（§7、RELEASE_RUNBOOK §5） | 主要接件人休假或超過時限時接手 | 同上 |
...
服務時段：**週一至週五 09:00–21:00**（Asia/Taipei；主要接件人蘇子傑 2026-09-24 提供）。「工作天」＝週一至週五。國定假日是否服務未指定，對外文案不寫假日服務。
...
| 超過時限 | 不設備援接件人：主要接件人每個工作天第一件事檢查逾時案件；超過 1 個工作天仍無法處理 → 暫停對外媒合入口（J-004 開關） |

新案件通知：MVP 由接件人每個工作天上午與下午各執行一次 `lead list --status NEW`。如需即時通知（email 等），另行決策，不在本版範圍。

使用者看到的文字需與實際承諾一致，例如：「Kareo 服務人員將於 1 個工作天內（週一至週五 09:00–21:00）致電與您確認需求。」
