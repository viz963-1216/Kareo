...
Owner: Jerry
Status: PROPOSED（D-06）；**接件人尚未指定（BLOCKED）**
...
| 角色 | 人選 | 責任 | 權限 |
|---|---|---|---|
| 主要接件人（Lead Operator） | **待 Jerry 指定真人並取得本人同意** | 依 §5 時程聯繫、更新狀態、記錄結果 | `lead:list`、`lead:show`、`lead:reveal-contact`（被指派案件）、`lead:update` |
| 備援接件人 | **待 Jerry 指定** | 主要接件人休假或超過時限時接手 | 同上 |
...
服務時段：週一至週五 09:00–18:00（國定假日除外）。**實際時段依指定的接件人可承諾時間調整後才能對外公告。**
...
使用者看到的文字需與實際承諾一致，例如：「Kareo 服務人員將於 1 個工作天內（週一至週五 09:00–18:00）致電與您確認需求。」
